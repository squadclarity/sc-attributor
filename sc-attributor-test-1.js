...
(function () {
  ...
  // CONSENT MANAGEMENT
  // -------------------------------------------------------
  const CONSENT_STORAGE_KEY = "_sc_consent";
  const CONSENT_COOKIE_NAME = "_sc_consent";
  const CONSENT_COOKIE_DAYS = 180;
  const CONSENT_ACCEPTED = "accepted";
  const CONSENT_DECLINED = "declined";

  function setBasicCookie(name, value, days = 180) {
    const expires = new Date(Date.now() + days * 864e5).toUTCString();
    document.cookie = `${name}=${encodeURIComponent(value)}; expires=${expires}; path=/; SameSite=Lax`;
  }

  function getBasicCookie(name) {
  ...
  function getConsentChoice() {
    try {
      return localStorage.getItem(CONSENT_STORAGE_KEY) || getBasicCookie(CONSENT_COOKIE_NAME);
    } catch {
      return getBasicCookie(CONSENT_COOKIE_NAME);
    }
  }

  function hasConsentDecision() {
    const choice = getConsentChoice();
    return choice === CONSENT_ACCEPTED || choice === CONSENT_DECLINED;
  }

  function hasAnalyticsConsent() {
    return getConsentChoice() === CONSENT_ACCEPTED;
  }
  ...
  function saveConsentChoice(choice) {
    ...
    setBasicCookie(CONSENT_COOKIE_NAME, choice, CONSENT_COOKIE_DAYS);
  }
  ...
  function injectConsentStyles() {
    ...
    style.textContent = `
      ...
      .sc-consent-panel-title {
        margin: 0 0 5px;
        font-size: 14px;
        font-weight: 700;
      }
      .sc-consent-panel-copy {
        margin: 0;
        font-size: 12px;
        line-height: 1.6;
        color: #475569;
      }
      .sc-consent-cookie-list {
        display: grid;
        gap: 8px;
        margin-top: 10px;
      }
      .sc-consent-cookie-item {
        display: grid;
  ...
  function removeConsentBanner() {
    const existing = document.getElementById("sc-consent-backdrop");
    if (existing) existing.remove();
  }

  function renderPrivacyLink() {
    if (document.getElementById("sc-privacy-link")) return;
    const btn = document.createElement("button");
    btn.id = "sc-privacy-link";
    btn.type = "button";
    btn.textContent = "Privacy Settings";
    btn.addEventListener("click", () => showConsentBanner(true));
    document.body.appendChild(btn);
  }

  function setConsentAndReload(choice) {
  ...
  function showConsentBanner(forceOpen = false) {
    onReady(() => {
      ...
      if (!forceOpen && hasConsentDecision()) return;
      if (document.getElementById("sc-consent-backdrop")) return;

      const currentChoice = getConsentChoice();
      const backdrop = document.createElement("div");
      backdrop.id = "sc-consent-backdrop";

      const card = document.createElement("div");
      card.id = "sc-consent-card";

      const statusText =
        currentChoice === CONSENT_ACCEPTED
          ? "Current setting: Analytics and attribution cookies accepted."
          : currentChoice === CONSENT_DECLINED
            ? "Current setting: Analytics and attribution cookies declined."
            : "Choose whether VDG PATH may set analytics and attribution cookies on this site.";

      card.innerHTML = `
        <div class="sc-consent-header">
          <div class="sc-consent-kicker">VDG PATH Tracking</div>
          <h2 class="sc-consent-title">Control analytics and attribution cookies</h2>
          <p class="sc-consent-copy">
            We use VDG PATH to understand how visitors reach the site, which pages they view,
            and whether they submit a form or call. These cookies help with attribution and reporting,
            and they are only enabled if you choose to allow them.
          </p>
        </div>
        <div class="sc-consent-body">
          <div class="sc-consent-panel">
            <h3 class="sc-consent-panel-title">What cookies are used?</h3>
            <p class="sc-consent-panel-copy">
              Necessary preferences are stored so we remember your choice. If you accept analytics,
              VDG PATH will also store visitor and session identifiers for attribution reporting.
            </p>
            <div class="sc-consent-cookie-list">
              <div class="sc-consent-cookie-item">
                <div>
                  <strong>_sc_consent</strong>
                  Necessary
                </div>
                <div>
                ...
                <div>
                  ...
                  Remembers whether you accepted or declined analytics and attribution tracking.
                </div>
              </div>
              <div class="sc-consent-cookie-item">
                <div>
                  <strong>_sc_vid</strong>
                  Analytics
                </div>
                <div>
                  <strong>Duration</strong>
                ...
                <div>
                  <strong>Purpose</strong>
                  Distinguishes repeat visitors so VDG PATH can connect journeys across visits.
                </div>
              </div>
              <div class="sc-consent-cookie-item">
                <div>
                  <strong>_sc_sid</strong>
                  Analytics
                </div>
                <div>
                  <strong>Duration</strong>
                ...
                <div>
                  <strong>Purpose</strong>
                  Groups activity into a browsing session for attribution and submission tracking.
                </div>
              </div>
              <div class="sc-consent-cookie-item">
                <div>
                  <strong>_sc_last_session</strong>
                  Local storage
                </div>
                <div>
                  <strong>Duration</strong>
                ...
                <div>
                  <strong>Purpose</strong>
                  Helps VDG PATH determine whether a visit is part of the same session.
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="sc-consent-actions">
          <div id="sc-consent-status">${statusText}</div>
          <div class="sc-consent-buttons">
            <button type="button" class="sc-btn sc-btn-secondary" id="sc-consent-decline">Decline analytics</button>
            <button type="button" class="sc-btn sc-btn-primary" id="sc-consent-accept">Accept analytics</button>
          </div>
        </div>
      `;
      ...
      card.querySelector("#sc-consent-accept").addEventListener("click", () => {
        setConsentAndReload(CONSENT_ACCEPTED);
      });

      card.querySelector("#sc-consent-decline").addEventListener("click", () => {
        saveConsentChoice(CONSENT_DECLINED);
        removeConsentBanner();
        renderPrivacyLink();
      });
    });
  ...
  if (!hasAnalyticsConsent()) {
    showConsentBanner(!hasConsentDecision());
    return;
  }
