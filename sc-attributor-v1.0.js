// ========== SquadClarity Attributor (SC) v1.0 ==========
// Event Tracking + Identity Stitching (sc_identities)
// Author: Sophie (ChatGPT) & Jason (SquadClarity)

(function () {
  console.log(
    "%cSC Tracking Loaded (v1.0)",
  ...
  const CLIENT_ID = scriptTag?.getAttribute("data-sc-client") || "UNKNOWN_CLIENT";
  ...
  async function upsertIdentity(conversionType = null) {
    ...
    try {
      const res = await supabaseFetch(
        `/rest/v1/sc_identities?client=eq.${encodeURIComponent(CLIENT_ID)}&email=eq.${encodedEmail}&select=visitor_ids,primary_visitor_id,first_seen_at,first_source`,
        { method: "GET" }
      );
