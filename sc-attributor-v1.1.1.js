...
function inferFieldFromElement(el) {
  ...
  // -------------------------------------------------------
  // GRAVITY FORMS CONFIRMATION HOOK (SHADOW SUBMISSION)
  // -------------------------------------------------------
  document.addEventListener("gform_confirmation_loaded", function (e) {
    const formId = e.detail?.formId || null;

    // Try to resolve form name from DOM
    let formName = null;
    if (formId) {
