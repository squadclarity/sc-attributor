// ========== Vision Design Attributor (VDA) v1.8.4 ==========
// v1.7.3 parity + Identity Stitching (vda_identities)
// Author: Sophie (ChatGPT) & Jason (Vision Design Group)

(function () {
  console.log(
    "%cVDA Tracking Loaded (v1.8.4 — v1.7.3 parity + Identity Stitching)",
  ...
  if (BOT_UA_PATTERNS.some((p) => ua.includes(p))) {
    console.log("VDA: bot / automated traffic ignored");
    return;
  }
  ...
  const CLIENT_ID = scriptTag?.getAttribute("data-vda-client") || "UNKNOWN_CLIENT";

  const SUPABASE_URL = "https://vjosgoqqqefmmagzazjf.supabase.co";
  const SUPABASE_ANON_KEY =
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZqb3Nnb3FxcWVmbW1hZ3phempmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjU0MDQ5MzQsImV4cCI6MjA4MDk4MDkzNH0._okZprKJ8WWQtIEDRhYSurJvb1GK4L8xk203uBS83NY";

  // Always initialize conversion rules container (parity with 1.7.3)
  window._vda_conversion_rules = [];
  ...
  // -------------------------------------------------------
  // SESSION + VISITOR MANAGEMENT (parity with 1.7.3)
  // -------------------------------------------------------
  let visitorId = getCookie("_vda_vid");
  if (!visitorId) {
    visitorId = uuid();
    setCookie("_vda_vid", visitorId);
  }

  let sessionId = getCookie("_vda_sid");
  const SESSION_TIMEOUT = 30 * 60 * 1000; // 30 minutes

  function startNewSession() {
    sessionId = uuid();
    setCookie("_vda_sid", sessionId);
    localStorage.setItem("_vda_last_session", Date.now());
  }

  const lastSessionTime = parseInt(localStorage.getItem("_vda_last_session") || "0");
  const now = Date.now();

  if (!sessionId || now - lastSessionTime > SESSION_TIMEOUT) {
    startNewSession();
  } else {
    localStorage.setItem("_vda_last_session", now);
  }
  ...
  function captureIdentity(field, value) {
    ...
    if (identity.email && !identity.first_seen_at) {
      ...
      identity.first_source = detectSource();
    }
...
function inferFieldFromElement(el) {
  ...
  // -------------------------------------------------------
  // IDENTITY UPSERT (vda_identities)
  ...
  async function upsertIdentity(conversionType = null) {
    ...
    const dedupeKey = `vda_ident_${identity.email}_${sessionId}_${conversionType ? "conv" : "base"}_${hasNames ? "named" : "anon"}`;
    ...
    try {
      const res = await supabaseFetch(
        `/rest/v1/vda_identities?client=eq.${encodeURIComponent(CLIENT_ID)}&email=eq.${encodedEmail}&select=visitor_ids,primary_visitor_id,first_seen_at,first_source`,
        { method: "GET" }
      );
      ...
      console.error("VDA identity read error:", err);
    }
    ...
    const payload = {
      ...
      first_source: existing?.first_source || identity.first_source || detectSource(),
      conversion_source: detectSource(),
      conversion_type: conversionType || null,

      first_seen_at: existing?.first_seen_at || identity.first_seen_at || new Date().toISOString(),
      conversion_at: conversionType ? new Date().toISOString() : null,
    ...
    try {
      if (existing) {
        const patchRes = await supabaseFetch(
          `/rest/v1/vda_identities?client=eq.${encodeURIComponent(CLIENT_ID)}&email=eq.${encodedEmail}`,
          {
            method: "PATCH",
            headers: {
        ...
        if (!patchRes.ok) {
          console.error("VDA identity PATCH failed:", await patchRes.text());
        }
      } else {
        const postRes = await supabaseFetch(`/rest/v1/vda_identities`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Prefer: "return=minimal"
        ...
        if (!postRes.ok) {
          console.error("VDA identity POST failed:", await postRes.text());
        }
      }
    } catch (err) {
      console.error("VDA identity upsert error:", err);
    }
  }

  // -------------------------------------------------
  // SEND CONVERSION EVENT (parity with 1.7.3)
  ...
  async function sendConversion(conversionType = "conversion", extra = {}) {
    ...
    const dedupeKey = `vda_conv_${conversionType}_${sessionId}`;
    if (sessionStorage.getItem(dedupeKey)) {
      console.log("VDA: duplicate conversion blocked", conversionType);
      return;
    }
    sessionStorage.setItem(dedupeKey, "1");
    ...
    console.log("VDA Conversion ->", payload);

    try {
      const res = await fetch(`${SUPABASE_URL}/rest/v1/vda_conversions`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          apikey: SUPABASE_ANON_KEY,
      ...
      console.error("VDA conversion error:", err);
    }
  ...
  // Manual API trigger: vdaConversion("contact_form")
  window.vdaConversion = (type = "conversion") => sendConversion(type);

  function autoDetectSuccessPage() {
    // left intentionally parity with 1.7.3 (currently disabled)
    ...
    //   console.log("VDA: auto success page match");
  ...
  async function checkSupabaseConversionRules() {
    ...
    try {
      const url = `${SUPABASE_URL}/rest/v1/vda_conversion_rules?client=eq.${CLIENT_ID}&enabled=eq.true`;
      ...
      if (!res.ok) {
        ...
        console.error("VDA rules fetch failed:", res.status, text);
        return;
      }

      const rules = await res.json();
      console.log("VDA rules loaded:", rules);

      window._vda_conversion_rules = rules;
      ...
      rules.forEach((rule) => {
        ...
        const ruleKey = `vda_rule_${rule.id}_${sessionId}`;
      ...
      console.error("VDA rule check error:", err);
    }
  }

  // EXTERNAL CLICK RULES (parity with 1.7.3)
  document.addEventListener("mousedown", (e) => {
    ...
    const rules = window._vda_conversion_rules;
  ...
  // -------------------------------------------------------
  // SUPABASE EVENT INSERT (parity with 1.7.3)
  // -------------------------------------------------------
  async function sendEvent(eventType, extra = {}) {
    ...
    console.log("VDA Event ->", payload);

    try {
      const res = await fetch(`${SUPABASE_URL}/rest/v1/vda_events`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          apikey: SUPABASE_ANON_KEY,
      ...
      if (!res.ok) {
        console.error("VDA event not stored:", await res.text());
      }
    } catch (err) {
      console.error("VDA tracking error:", err);
    }
  }

  // Phone clicks (parity with 1.7.3)
  function trackPhoneClicks() {
    document.querySelectorAll('a[href^="tel:"]').forEach((link) => {
      link.addEventListener("mousedown", () => {
        window.vdaConversion("phone_call_click");
      });
    });
  }
  window.addEventListener("load", trackPhoneClicks);

  // -------------------------------------------------------
  // FIRE PAGEVIEW + INIT RULES (parity with 1.7.3)
  // -------------------------------------------------------
  sendEvent("pageview");
  ...
  // -------------------------------------------------------
  // EXPOSE DEBUG INFO (parity with 1.7.3 + identity)
  // -------------------------------------------------------
  window.VDA = {
    version: "1.8.4",
    client: CLIENT_ID,
