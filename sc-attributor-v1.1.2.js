...
function inferFieldFromElement(el) {
  ...
  // -------------------------------------------------------
  // GRAVITY FORMS CONFIRMATION HOOK (jQuery-based)
  // -------------------------------------------------------
  (function waitForGF() {
    if (!window.jQuery) {
      setTimeout(waitForGF, 50);
      return;
    }

    jQuery(document).on(
      "gform_confirmation_loaded",
      function (event, formId, entryId) {

        // Resolve form name if possible
        let formName = null;
        ...
        // Capture longest text field as message
        let message = null;
        let maxLen = 0;
        document
        ...
        console.log("✅ SC Shadow Submit (GF)", {
          formId,
          entryId,
          formName,
          message
