...
function inferFieldFromElement(el) {
  ...
  // -------------------------------------------------------
  // GRAVITY FORMS OFFICIAL JS HOOK (ALWAYS FIRES)
  // -------------------------------------------------------
  (function waitForGFCore() {
    if (!window.gform || !window.gform.addAction) {
      setTimeout(waitForGFCore, 50);
      return;
    }

    window.gform.addAction(
      "gform_confirmation_loaded",
      function (formId, entryId) {

        // Resolve form element
        const formEl = document.getElementById(`gform_${formId}`);

        // Resolve form name if available
        let formName = null;
        if (formEl) {
          const titleInput = formEl.querySelector(
            'input[name="gform_form_title"]'
          );
          if (titleInput && titleInput.value) {
            formName = titleInput.value;
          }
        }

        // Capture message from *this form only*
        let message = null;
        let maxLen = 0;
        ...
        console.log("✅ SC Shadow Submit (GF CORE)", {
          formId,
          entryId,
          formName,
          message
